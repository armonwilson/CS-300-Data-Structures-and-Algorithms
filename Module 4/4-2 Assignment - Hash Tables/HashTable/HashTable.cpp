...
void HashTable::Insert(Bid bid) {
    ...
    newNode->key = hash(atoi(bid.bidId.c_str()));
    // retrieve node using key
    Node* oldNode = &nodes[newNode->key];
    // if no entry found for the key
    if (oldNode == nullptr) {
        // assign this node to the key position
        oldNode->bid = newNode->bid;
...
void HashTable::PrintAll() {
    ...
    for (int i = 0; i < nodes.size(); i++) {
        ...
        if (current != nullptr && current->key != UINT_MAX) {
            ...
            cout << current->key << " | " << current->bid.bidId << " | " << current->bid.title << " | " << current->bid.fund << endl;
